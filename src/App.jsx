...
const ALARMS = ['1st Alarm','2nd Alarm','3rd Alarm','4th Alarm','5th Alarm','6th Alarm'];
...
const InvTag  =({value})=>{const cl=value==='Structural'?'tag-s':value==='Vehicular'?'tag-v':'tag-n';return<span className={`tag ${cl}`}>{value}</span>;};
...
export default function App() {
  ...
  const yearData=useMemo(()=>{
    ...
    incidents.forEach(inc=>{
      ...
      if(!map[yr])map[yr]={year:yr,total:0,structural:0,nonstructural:0,vehicular:0,damage:0,injured:0,casualty:0,u1:0,u2:0};
      const y=map[yr];y.total++;
      if(inc.involved==='Structural')y.structural++;
      else if(inc.involved==='Vehicular')y.vehicular++;
      else y.nonstructural++;
  ...
  const yrTotals=yearData.reduce((acc,y)=>{Object.keys(acc).forEach(k=>acc[k]+=y[k]);return acc;},{total:0,structural:0,nonstructural:0,vehicular:0,damage:0,injured:0,casualty:0,u1:0,u2:0});

  // ══ STATS ═══════════════════════════════════════════════════════════════
  const total=incidents.length,structural=incidents.filter(i=>i.involved==='Structural').length,vehicular=incidents.filter(i=>i.involved==='Vehicular').length;
            ...
            <>
              <div className="stats-grid">
                {[
                  ...
                  {label:'Non-Structural',val:total-structural-vehicular,sub:'fires'},
                  {label:'Vehicular',val:vehicular,sub:'fires'},
              ...
              <div className="search-wrap">
                ...
                <div>
                  ...
                  <div className="pills">
                    ...
                    <span className="pill" onClick={()=>{setFilterInv('Vehicular');setIncPage(1);}}>Vehicular only</span>
                  </div>
                </div>
              </div>
          ...
          {page==='bulk'&&(
            <>
              ...
              <div className="info-bar">
                ...
                <span>Accepted: <strong>CSV</strong>, <strong>TSV</strong>, or <strong>Excel (.xlsx)</strong>. Required columns: <em>Time & Date, Location, Involved, Occupancy, Damage, Injured, Casualty, Station, Engine, Alarm</em>. Date format: <code>DD Month YYYY HHMMH</code>. Involved values: <strong>Structural</strong>, <strong>Non-Structural</strong>, <strong>Vehicular</strong>.</span>
              </div>
                ...
                <div className="yr-table-wrap">
                  {!yearData.length?<div className="yr-empty">No incident records yet.</div>:(
                    <table className="yr-table">
                      <thead><tr><th>Year</th><th className="num">Total</th><th className="num">Structural</th><th className="num">Non-Structural</th><th className="num">Vehicular</th><th>Est. Total Damage</th><th className="num">Injured</th><th className="num">Casualties</th><th className="num">By {personnel[0].name}</th><th className="num">By {personnel[1].name}</th></tr></thead>
                      <tbody>{yearData.map(y=><tr key={y.year}><td className="year-cell">{y.year}</td><td className="num">{y.total}</td><td className="num">{y.structural}</td><td className="num">{y.nonstructural}</td><td className="num">{y.vehicular}</td><td className="dmg-cell">₱{y.damage.toLocaleString()}</td><td className="inj-cell">{y.injured}</td><td className="cas-cell">{y.casualty}</td><td className="num">{y.u1}</td><td className="num">{y.u2}</td></tr>)}</tbody>
                      <tfoot><tr><td className="tl">All Years</td><td style={{textAlign:'center'}}>{yrTotals.total}</td><td style={{textAlign:'center'}}>{yrTotals.structural}</td><td style={{textAlign:'center'}}>{yrTotals.nonstructural}</td><td style={{textAlign:'center'}}>{yrTotals.vehicular}</td><td>₱{yrTotals.damage.toLocaleString()}</td><td style={{textAlign:'center',color:'#ffc864'}}>{yrTotals.injured}</td><td style={{textAlign:'center',color:'#ff9090'}}>{yrTotals.casualty}</td><td style={{textAlign:'center'}}>{yrTotals.u1}</td><td style={{textAlign:'center'}}>{yrTotals.u2}</td></tr></tfoot>
                    </table>
                  )}
                </div>
              </div>
                  ...
                  return<div key={p.id} className="personnel-card">
                    ...
                    <div className="pc-body">
                      <div className="pc-stat-row">
                        ...
                        <div className="pc-stat"><div className="pc-stat-n">{incidents.filter(i=>i.inputter===p.name&&i.involved==='Vehicular').length}</div><div className="pc-stat-l">Vehicular</div></div>
                      </div>
          ...
          <div className="modal">
            ...
            <div className="modal-body">
              <div className="form-grid">
                ...
                <div className="fg">
                  <label className="flbl">Involved</label>
                  <select className="fsel" value={form.involved} onChange={e=>setF('involved',e.target.value)}>
                    <option>Structural</option>
                    <option>Non-Structural</option>
                    <option>Vehicular</option>
                  </select>
